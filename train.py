...
from misc import AvgMeter, check_mkdir, CriterionKL3, CriterionKL, CriterionPairWise, CriterionStructure, PixelInfoNCELoss
from F3Net.net import F3Net, warp
...
if args['structure']:
    ...
    # criterion_contra = PixelInfoNCELoss().cuda()
...
def main():
    ...
    net = F3Net(cfg=None).cuda(device_id).train()
...
def train_single(net, inputs, flows, labels, optimizer, curr_iter, teacher):
    ...
    optimizer.zero_grad()
    out1u, out2u, out3, out2r, out3r, out4r, out5r, out2f, out3f, out4f = net(inputs, flows, architecture=get_random_cand())
    ...
    # print('labels count:', labels_contra.nonzero())
    ...
    loss10 = criterion_str(out3, labels)
    # loss11 = criterion_str(out2a, labels)
    # if torch.isnan(loss_contra):
    total_loss = (loss0 + loss1) / 2 + loss2 / 2 + loss3 / 4 + loss4 / 8 + \
                 loss5 / 16 + loss7 / 4 + loss8 / 8 + loss9 / 16 + loss10 / 2
    # else:
    #     total_loss = (loss0 + loss1) / 2 + loss2 / 2 + loss3 / 4 + loss4 / 8 + loss5 / 16 + 0.1 * loss_contra
...
def train_single2(net, inputs, labels, optimizer, curr_iter):
    ...
    out1u, out2u, out3, out2r, out3r, out4r, out5r = net(inputs, architecture=get_random_cand())
    ...
    loss6 = criterion_str(out3, labels)

    total_loss = (loss0 + loss1) / 2 + loss2 / 2 + loss3 / 4 + loss4 / 8 + loss5 / 16 + loss6 / 2
