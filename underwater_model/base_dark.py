...
class Base(nn.Module):
    def __init__(self, channels):
        ...
        self.block2_rgb = sa_layer(channels[0], 16)
        self.block3_rgb = Conv(channels[1], channels[1], 3, 1, 1, affine=True, upsample=False)
        self.block4_rgb = DoubleAttentionLayer(channels[1], channels[1], channels[1])
        self.block5_rgb = DilConv(channels[2], channels[2], 3, 1, 4, 4, affine=True, upsample=False)
        self.block6_rgb = Conv(channels[2], channels[2], 3, 1, 1, affine=True, upsample=False)

        self.block1_lab = eca_layer(3)
        self.block2_lab = DoubleAttentionLayer(channels[0], channels[0], channels[0])
        self.block3_lab = eca_layer(3)
        self.block4_lab = eca_layer(3)
        self.block5_lab = Identity(False)
        self.block6_lab = Identity(False)
