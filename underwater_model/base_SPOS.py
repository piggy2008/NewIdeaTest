...
class Base(nn.Module):
    ...
    def forward(self, rgb, hsv, lab, select):
        ...
        x_rgb2 = self.block3(x_rgb2, select[0])
        x_rgb2 = self.block4(x_rgb2, select[1])
        x_rgb3 = F.max_pool2d(x_rgb2, kernel_size=3, stride=2, padding=1)
        x_rgb3 = self.conv3_rgb(x_rgb3)
        x_rgb3 = self.block5(x_rgb3, select[2])
        x_rgb3 = self.block6(x_rgb3, select[3])
        ...
        x_hsv2 = self.block3(x_hsv2, select[4])
        x_hsv2 = self.block4(x_hsv2, select[5])
        x_hsv3 = F.max_pool2d(x_hsv2, kernel_size=3, stride=2, padding=1)
        x_hsv3 = self.conv3_hsv(x_hsv3)
        x_hsv3 = self.block5(x_hsv3, select[6])
        x_hsv3 = self.block6(x_hsv3, select[7])
