...
class TransformerClassifier(nn.Module):
    def __init__(self,
        ...
        if not seq_pool:
            ...
            self.attention_pool = nn.Linear(self.embedding_dim, self.image_size * self.image_size)
    ...
    def forward(self, x):
        ...
        if self.seq_pool:
            ...
            x = x.permute(0, 2, 1).reshape(-1, self.embedding_dim, self.image_size, self.image_size)
        else:
            # x = x.permute(0, 2, 1).reshape(1, self.embedding_dim, self.image_size, self.image_size)
            x = x[:, 0]
