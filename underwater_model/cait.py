...
class CaiT(nn.Module):
    ...
    def forward(self, img):
        ...
        x_cls = self.cls_transformer(cls_tokens, context = x)

        x = self.recovery(torch.cat([x, x_cls], dim=1).permute(0, 2, 1)).reshape(b, -1, self.image_height, self.image_width)
        return x
...
if __name__ == '__main__':
    v = CaiT(
        ...
        num_classes=1000,
        dim=128,
        depth=1,  # depth of transformer for patch to patch attention only
